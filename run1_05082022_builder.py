...
#Do the git-ing
cmd='git commit -a -m "Commit before run run1_05082022" > /dev/null'
print(cmd)
...
open('run1_05082022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run1_05082022.task','w')
template_file='run1_05082022.template'
template_contents=open(template_file,'r').read()

vnum=0

for L in xrange(2): # number of parameters to be sweeped over.
	...
	task_file.write('bash run1_05082022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run1_05082022.sbatch.template','r').read()
...
open('run1_05082022.sbatch','w').write(contents)
