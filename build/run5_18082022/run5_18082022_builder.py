...
#Do the git-ing
cmd='git commit -a -m "Commit before run run5_18082022" > /dev/null'
print(cmd)
...
for L in xrange(32,64):
	...
	contents=contents.replace('*111*',str(0.0012170226130653265*(L-32)+0.038944723618090454))
	...
	task_file.write('bash run5_18082022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run5_18082022.sbatch.template','r').read()
...
open('run5_18082022.sbatch','w').write(contents)
