...
#Do the git-ing
cmd='git commit -a -m "Commit before run run4_18082022" > /dev/null'
print(cmd)
...
open('run4_18082022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run4_18082022.task','w')
template_file='run4_18082022.template'
...
for L in xrange(32):
	...
	task_file.write('bash run4_18082022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run4_18082022.sbatch.template','r').read()
...
open('run4_18082022.sbatch','w').write(contents)
