...
#Do the git-ing
cmd='git commit -a -m "Commit before run run7_18082022" > /dev/null'
print(cmd)
...
open('run7_18082022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run7_18082022.task','w')
template_file='run7_18082022.template'
...
for L in xrange(1):
	...
	task_file.write('bash run7_18082022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run7_18082022.sbatch.template','r').read()
...
open('run7_18082022.sbatch','w').write(contents)
