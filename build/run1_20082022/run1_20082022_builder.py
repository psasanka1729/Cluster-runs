...
#Do the git-ing
cmd='git commit -a -m "Commit before run run1_20082022" > /dev/null'
print(cmd)
...
open('run1_20082022.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('run1_20082022.task','w')
template_file='run1_20082022.template'
template_contents=open(template_file,'r').read()

vnum=0

eps_list = [0.0,0.011,0.012,0.016,0.019]
for L in xrange(5):
	...
	task_file.write('bash run1_20082022_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('run1_20082022.sbatch.template','r').read()
...
open('run1_20082022.sbatch','w').write(contents)
